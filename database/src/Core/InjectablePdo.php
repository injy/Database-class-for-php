...
class InjectablePdo
{
    ...
    public function getPdo(int $dbid): \PDO
    {
        if (!isset(self::$pdoPool[$dbid])) {
            try {
                ...
                // 验证必要配置
                $this->validateDbConfig($dbInfo, $dbid);
                
                $dsn = "mysql:host={$dbInfo['host']};port={$dbInfo['port']};dbname={$dbname};charset=utf8mb4";
                self::$pdoPool[$dbid] = new \PDO($dsn, $dbInfo['user'], $dbInfo['pw'], self::PDO_OPTIONS);
                
                write_log("PDO连接已建立: dbid={$dbid}, host={$dbInfo['host']}", self::log_type);
                
            } catch (PDOException $e) {
                write_log("PDO连接失败: dbid={$dbid}, error={$e->getMessage()}", self::log_type);
                throw new \RuntimeException("数据库连接失败: " . $e->getMessage(), 0, $e);
            }
        }
        
        // 检查连接是否有效
        if (!$this->isConnectionAlive(self::$pdoPool[$dbid])) {
            unset(self::$pdoPool[$dbid]);
            return $this->getPdo($dbid); // 重新连接
        }
    ...
    /** 验证数据库配置 */
    private function validateDbConfig(array $dbInfo, int $dbid): void
    {
        $required = ['host', 'port', 'user', 'pw'];
        foreach ($required as $field) {
            if (!isset($dbInfo[$field]) || empty($dbInfo[$field])) {
                throw new \InvalidArgumentException("数据库配置缺失字段: {$field} (dbid={$dbid})");
            }
        }
        
        if (!is_numeric($dbInfo['port']) || $dbInfo['port'] <= 0) {
            throw new \InvalidArgumentException("数据库端口无效: {$dbInfo['port']} (dbid={$dbid})");
        }
    }
